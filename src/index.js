...
detectSystemInfo((error, architecture, platform) => {
  ...
  if (platform === 'android') {
    ...
    console.log(
      `🚀 [@hiudyy/ytdl] Modul diinisialisasi dengan Python untuk Android.`,
    );
    return;
  }
  ...
  console.log(
    `✅ [@hiudyy/ytdl] Modul berhasil diinisialisasi pada arsitektur: ${architecture}.`,
  );
});
              ...
              async (error, stdout, stderr) => {
                if (error) {
                  if (HiudyyDLPath.includes('hiudyydl_py')) {
                    execFile(
                      'python',
                      [HiudyyDLPath, ...args, url],
                      async (pyErr, pyStdout, pyStderr) => {
                        if (pyErr) {
                          return reject(
                            `Hiudyydl error (Python): ${pyStderr || pyErr.message}`,
                          );
                        }
