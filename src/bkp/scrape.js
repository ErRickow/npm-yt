...
const baseQualities = ["250p", "360p", "480p", "720p", "1080p", "1440p", "2160p"];
...
class Video {
    ...
    getMetadataMatches() {
        ...
        this.metadataMatches = metadataText.match(/(\d+min|\d+p|\d[\d.,]*\s*[views]*)/g);
    }
    ...
    get length() {
        const length = this.metadataMatches ? this.metadataMatches[0].replace("min", "").trim() : "";
        return length;
    }
...
class User {
    ...
    async getVideos() {
        ...
        const response = await axios.get(url, { headers: consts.HEADERS });
        return response.data.videos.map(video => new Video(`https://www.xnxx.com${video.u}`));
    }
...
class Client {
    ...
    static search(query, uploadTime = "", length = "", searchingQuality = "", limit = 5) {
