  ...
  const testResults = await Promise.allSettled(
    testCases.map(async (test) => {
      try {
        ...
        return `${test.name}: ❌ (${err.message})`;
      }
    })
  );

  console.log(testResults.map(result => result.value || result.reason).join('\n'));
}

testScraper().catch(err => console.error('Fatal error:', err.stack));